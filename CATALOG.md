...
## Changelog
- **v9.7 — the arapaima lands**: curator-supplied arapaima model replaces the GLB shark as the tank predator (static mesh + procedural sway, gulp-feeding — no jaw needed); mobile variant (2.9 MB) served; wall bounds now body-aware (no more snout through the glass); frustumCulled=false on GLB meshes fixes the shadow popping on the roof; Dusk Lancer procedural predator stays shelved in predator.js.
- **v9.6 — tendencies, not rules**: minnow behaviour rewritten as weighted tendencies with per-fish variance (space/social/bold/dart/depth traits); turn-rate-limited forward-only swimming (no sideways gliding); soft moonstone aversion field replaces the hard clamp; tank radius ×1.66 (5.2 m — a walkable hallway around a massive tank); exterior glass sheen + plinth uplights (bright hall outside, night inside — shadow rig preserved); species school anchors + GLB prey same-species social weighting; hunt camera preset.
- **v9.5 — minnow behaviour model (research-driven)**: replaced the shell-anchor ball with an emergent schooling model — repulsion/alignment/attraction zones, boldness personalities, two remembered shoalmates per fish, contagious panic waves, C-start darts, hunger-driven foraging; hard separation for minnow↔minnow and minnow↔troupe (no clipping); the ball drifts on its own anchor and keeps 1.35 m off the moonlight; shark count self-heals on load; tools/probe-hunt.mjs diagnostic added.
