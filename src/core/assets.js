...
export async function loadFishPack(dir='assets/fish'){
  ...
  try{
    ...
    assets.fish=await Promise.all(list.map(name=>new Promise((resolve,reject)=>loader.load(dir+'/'+name,g=>resolve({file:name,scene:g.scene,animations:g.animations||[]}),undefined,reject))));
