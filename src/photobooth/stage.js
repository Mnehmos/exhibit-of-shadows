...
export function createSpecimenStage({scene,renderer,onChange=()=>{}}){
  ...
  function mount(entry,nextIndex){
    ...
    const model=skClone(entry.scene),centerer=new THREE.Group(),alignment=new THREE.Group();centerer.add(model);alignment.add(centerer);specimenRoot.add(alignment);
    ...
    const meshes=cloneMaterials(model,renderer),clip=(entry.animations||[]).find(a=>/swim/i.test(a.name))||(entry.animations||[])[0]||null;
    if(clip){mixer=new THREE.AnimationMixer(model);mixer.clipAction(clip).play();}
    current={entry,alignment,model,nativeSize,meshes,fit};userYaw=0;userPitch=0;zoom=1;applyPose();
    onChange({index,count:entries.length,name:displayName(entry.file),file:entry.file,animations:entry.animations?.length||0,meshes,nativeSize});document.body.dataset.boothState='settled';return true;
  }
  ...
  function update(dt){if(mixer)mixer.update(dt);if(spin&&current){userYaw=(userYaw+dt*.22)%(Math.PI*2);applyPose();}}
  function stats(){return current?{file:current.entry.file,index,count:entries.length,animations:current.entry.animations?.length||0,meshes:current.meshes,view,spin}:null;}
